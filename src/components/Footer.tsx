...
export default function Footer() {
  return (
    <footer className="h-full w-full bg-[#1F1F1F] md:h-fit">
      <Margin>
        <div className="flex w-full flex-col items-center justify-between text-neutral-300 md:flex-row md:gap-0 md:py-0">
            ...
            <div className="flex items-center gap-3">
              {iconsConst.map((icon) => (
                <Link href={icon.href} key={icon.href}>
                  <Icons path={icon.path} width={35} color="#d4d4d4" />
                </Link>
              ))}
            </div>
          </div>
